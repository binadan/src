...
public class HostManagementAction extends BaseAction {
	...
	public String del() {
		...
		try {
			HostManagement hostToDelete = this.hostManagementService.findById(M_ID);
			// msg=this.paymentManagementService.delPay(M_ID);
			msg = this.hostManagementService.delHostManagement(M_ID);
			 msg=this.paymentManagementService.delPayByHostManualId(hostToDelete.getM_MANUAL());
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
			return ERROR;
