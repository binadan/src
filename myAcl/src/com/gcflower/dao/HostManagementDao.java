...
public List<HostManagement> findHosts(HostManagement hostManagement){
		
		return (List<HostManagement>)getHibernateTemplate().find("from HostManagement where M_KEFUZHANGHAO = '"+hostManagement.getM_KEFUZHANGHAO()+"'");
		/*if(ul !=null && ul.size()>=1){
			return (HostManagement)ul.get(0);
		}
		return null;*/
